...
def _run_backfill(*, args: argparse.Namespace, paths: OpenSourceLivePaths, run_id: str) -> None:
    ...
    sec_client = SecCompanyFactsClient(
        ...
        cache_dir=Path(__file__).resolve().parents[3] / "data" / "open_source" / "_cache" / "sec_companyfacts",
    )
    sec_filing_client = SecFilingFactsClient(
        user_agent=args.user_agent,
        cache_dir=Path(__file__).resolve().parents[3] / "data" / "open_source" / "_cache" / "sec_filing",
    )
