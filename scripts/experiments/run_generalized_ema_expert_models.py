...
class GeneralizedEmaModelConfig:
...
def _latest_feature_frame(output_dir: Path) -> Path:
    candidates = sorted(output_dir.glob("generalized_ema_expert_frame_*/generalized_ema_expert_frame.parquet"))
    if not candidates:
        raise FileNotFoundError("No generalized EMA expert frame found. Run build_generalized_ema_expert_frame.py first.")
    return candidates[-1]


def _feature_columns(frame: pl.DataFrame) -> list[str]:
    features = [column for column in frame.columns if column.startswith("learned_ema_expert_")]
    if not features:
        raise ValueError("No learned EMA expert features found.")
    return features
...
def _train_predict_classifier(
    *,
    ...
    model = _train_xgb(xgb, _matrix(train_df, features), y_train, params=_classifier_params(seed), rounds=250)
...
def _train_predict_regression(*, xgb, train_df: pl.DataFrame, test_df: pl.DataFrame, features: Sequence[str], seed: int) -> np.ndarray:
    ...
    model = _train_xgb(xgb, _matrix(train_df, features), y_train, params=_regression_params(seed), rounds=250)
    return _predict_xgb(xgb, model, _matrix(test_df, features))
...
def run(config: GeneralizedEmaModelConfig) -> Path:
    xgb = load_xgboost()
    ...
    run_dir = config.output_dir / f"generalized_ema_expert_models_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for position, window in enumerate(windows, start=1):
        ...
        prediction_frames.append(
            test_df.select(["ticker", "year_month", "holding_month", "future_excess_return", "legacy_selected"] + features)
            .with_columns(
                pl.Series(
                    "generalized_classifier_gt5",
                ...
                pl.Series(
                    ...
                    _train_predict_regression(xgb=xgb, train_df=train_df, test_df=test_df, features=features, seed=seed),
                    dtype=pl.Float64,
                ),
                pl.Series(
                    "generalized_rank_pairwise",
                    _train_predict_ranker(xgb=xgb, train_df=train_df, test_df=test_df, features=features, seed=seed),
                    dtype=pl.Float64,
                ),
    ...
    model_cols = [
        "learned_ema_expert_vote_count",
        "learned_ema_expert_score_sum",
        "generalized_classifier_gt5",
        "generalized_classifier_gt0",
        "generalized_regression",
        "generalized_rank_pairwise",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train future-return models on generalized EMA expert features.")
