...
| `portfolio_boosting_exact_ema_variants` | allocation autonome : trois variantes pure boosting sur les memes EMA exactes ; `boost_rank_pairwise` apprend un ranking mensuel XGBoost natif, `boost_rank_robust_active` regresse le rang futur avec objectif Optuna penalise risque, `boost_two_head_return_risk` combine trois predictions boosting proba top10 + rendement attendu - downside | memes 215 variables EMA exactes que `portfolio_boosting_exact_ema_rank` ; aucune cible Legacy, aucun score EMA brut, aucun blend momentum ; note : le ranker pairwise utilise XGBoost natif car `mlcraft` n'expose pas encore de `TaskType` ranking | backtest 2015+ `2015-02` a `2026-04`, run `outputs/portfolio_boosting_exact_ema_variants_20260705_153914`, 135 mois, 2 trials Optuna/fold | meilleur rendement = `boost_two_head_return_risk_top_5` +1062.3%, CAGR 24.4%, Sharpe 0.53, DD -40.8%, proche Legacy en rendement mais trop risque ; meilleur compromis a creuser = `boost_rank_robust_active_top_10` +502.1%, CAGR 17.3%, Sharpe 0.56, DD -30.2% ; pairwise top10 +223.6%, sous SPY |
...
- Diagnostic ajoute le 2026-06-27 soir : le boosting seul n'est pas mort, mais
  ...
  simple de trials.
- Diagnostic ajoute le 2026-07-05 : sur les 215 EMA exactes, le pure boosting
  progresse surtout quand l'objectif ressemble davantage a l'allocation. Le
  modele `boost_two_head_return_risk` approche le rendement de Legacy en top5
  (+1062.3% vs +1118.1%) mais paie ce resultat par un max drawdown beaucoup
  trop profond (-40.8% vs -23.5%) et un Sharpe faible (0.53 vs 0.93). Le
  candidat le plus propre pour continuer est donc `boost_rank_robust_active`,
  pas parce qu'il bat Legacy, mais parce qu'il bat SPY en rendement/CAGR avec
  un risque moins explosif que le two-head.
...
### Fiche du run `portfolio_boosting_exact_ema_variants`

Run principal : `outputs/portfolio_boosting_exact_ema_variants_20260705_153914`.

But : tester trois idees pure boosting sur la meme data que le meilleur run
recent, c'est-a-dire les 215 variables EMA exactes calculables au mois de
decision.

Ce qui est commun aux trois variantes :

- pas de cible Legacy ;
- pas de score EMA brut en sortie ;
- pas de blend avec une regle deterministe ;
- prediction out-of-sample walk-forward, train passe, validation temporelle,
  test mois suivant ;
- portefeuille construit ensuite en prenant le top N des predictions du mois.

Modeles :

| modele | ce qui est appris | variables | selection |
|---|---|---|---|
| `boost_rank_pairwise` | ordre relatif futur des actions dans chaque mois via XGBoost `rank:pairwise` groupe par mois | 215 EMA exactes : ratios EMA observes dans les blocs Legacy, rangs mensuels, z-scores mensuels, flags top/bottom quartile | top N du score de ranking predit |
| `boost_rank_robust_active` | rang percentile mensuel futur de `future_excess_return`, avec Optuna qui maximise un rendement actif top10 penalise par volatilite, drawdown et retard vs SPY | memes 215 variables EMA exactes | top N de la prediction de rang futur |
| `boost_two_head_return_risk` | trois predictions boosting : probabilite d'etre dans le top 10% futur, rendement relatif futur attendu, downside futur attendu | memes 215 variables EMA exactes | score combine uniquement avec predictions boosting : proba + rendement attendu - downside |

Point technique : `boost_rank_pairwise` utilise XGBoost natif, pas `mlcraft`,
car la version locale de `mlcraft` ne fournit pas encore de tache ranking
groupee. Les deux autres variantes utilisent `mlcraft`.

Resultat backtest 2015+ (`2015-02` a `2026-04`) :

| modele | total return | CAGR | Sharpe | max DD | lecture |
|---|---:|---:|---:|---:|---|
| `Combined_Frequency` | +1118.1% | 24.9% | 0.93 | -23.5% | reference Legacy a battre |
| `Combined_Equal` | +851.1% | 22.2% | 0.85 | -23.6% | autre reference Legacy |
| `boost_two_head_return_risk_top_5` | +1062.3% | 24.4% | 0.53 | -40.8% | proche Legacy en rendement, mais risque trop eleve |
| `boost_two_head_return_risk_top_7` | +828.5% | 21.9% | 0.51 | -35.3% | rendement fort, risque encore trop eleve |
| `boost_rank_robust_active_top_10` | +502.1% | 17.3% | 0.56 | -30.2% | meilleur candidat propre a creuser |
| `boost_rank_robust_active_top_20` | +422.5% | 15.8% | 0.58 | -31.9% | plus diversifie mais moins rentable |
| `SPY` | +332.2% | 13.9% | 0.79 | -23.9% | reference marche |
| `boost_rank_pairwise_top_10` | +223.6% | 11.0% | 0.44 | -27.9% | ranking pairwise seul insuffisant |

Resultat prediction top K :

- `boost_two_head_return_risk` a le meilleur rendement relatif moyen du top5 :
  `+1.16%` par mois en excess return, mais seulement `49.2%` de hit rate
  positif. Le signal paie par quelques gros mois, pas par une precision
  reguliere.
- `boost_rank_robust_active` est plus coherent au top10 : `+0.37%` par mois en
  excess return et `50.6%` de hit rate positif.
- `boost_rank_pairwise` ne transforme pas les EMA exactes en panier rentable :
  excess return moyen negatif sur tous les top K testes.

Decision :

- ne pas retenir `boost_two_head_return_risk` tel quel pour allocation reelle :
  il montre qu'il y a du signal boosting, mais le portefeuille top5 est trop
  concentre et trop volatile ;
- poursuivre `boost_rank_robust_active` avec plus de trials warm-startes et une
  penalisation portefeuille plus directe ;
- garder `boost_rank_pairwise` comme diagnostic, pas comme meilleur candidat
  courant.
...
- `outputs/portfolio_boosting_exact_ema_variants_20260705_153914`
...
Cette etape a ete executee ensuite dans le run exact ci-dessous.
...
- periode test : `2015-02` a `2026-04`
- EMA exactes generees : 43 couples observes dans les blocs `Legacy_Optuna_*`
- source EMA : `outputs/checkpoints_open_source_20260607/polars_final_price_vs_index.parquet`
- formule : `EMA(close_vs_index, n_short) / EMA(close_vs_index, n_long)`,
  alignee sur le mois de holding via le mois de signal precedent, comme Legacy
- EMA primaire du mois : couple `n_short/n_long` du `selected_model` atomique
  Legacy dominant du mois, c'est-a-dire celui qui a le plus de lignes selectionnees
  dans les sorties `Legacy_Optuna_*`
- point de vigilance : on utilise la configuration EMA Legacy dominante comme
  ancre, mais pas les tickers selectionnes par Legacy comme target ou feature.

Backtest exact vs Legacy :

| modele | rendement total | CAGR | Sharpe | max DD | vol mensuelle | mois positifs |
|---|---:|---:|---:|---:|---:|---:|
| Legacy `Combined_Frequency` | 1118.1% | 24.9% | 0.93 | -23.5% | 7.1% | 63.0% |
| Legacy `Combined_Equal` | 851.1% | 22.2% | 0.85 | -23.6% | 6.9% | 60.0% |
| EMA exacte + residu top 10 | 475.6% | 16.8% | 0.40 | -51.3% | 10.8% | 55.6% |
| EMA exacte + residu top 20 | 433.3% | 16.0% | 0.48 | -35.4% | 8.4% | 60.0% |
| SPY | 332.2% | 13.9% | 0.79 | -23.9% | 4.4% | 69.6% |
| EMA exacte + residu top 30 | 326.2% | 13.8% | 0.43 | -39.4% | 7.9% | 58.5% |
| EMA exacte + residu top 50 | 264.9% | 12.2% | 0.41 | -42.7% | 7.2% | 63.0% |
| EMA exacte seule top 50 | 180.8% | 9.6% | 0.35 | -41.6% | 6.3% | 63.7% |
| EMA exacte seule top 20 | 149.8% | 8.5% | 0.29 | -36.4% | 6.5% | 60.0% |

Metriques prediction exactes sur test :

| modele | RMSE | MAE | R2 | Pearson | Spearman |
|---|---:|---:|---:|---:|---:|
| EMA exacte seule | 0.0778 | 0.0556 | -0.0066 | 0.0090 | 0.0243 |
| EMA exacte + residu | 0.0778 | 0.0555 | -0.0065 | 0.0109 | 0.0241 |

Metriques top K exactes sur test :

| modele | top 10 excess | top 10 hit-rate | top 20 excess | top 20 hit-rate | top 50 excess | top 50 hit-rate |
|---|---:|---:|---:|---:|---:|---:|
| EMA exacte seule | -0.42% | 47.4% | -0.33% | 48.0% | -0.26% | 48.1% |
| EMA exacte + residu | 0.51% | 50.5% | 0.29% | 49.7% | -0.04% | 48.3% |

Lecture du run exact :

- la correction etait necessaire : le run proxy ne testait pas les vraies EMA
  Legacy arbitraires ;
- mais l'EMA exacte dominante seule ne suffit toujours pas a reproduire la
  performance Legacy ;
- le residu apporte le vrai gain : top20 passe de `+149.8%` a `+433.3%` ;
- top10 a le meilleur rendement (`+475.6%`) mais le drawdown `-51.3%` est trop
  eleve ;
- top20 est le meilleur compromis brut de ce run, mais reste sous Legacy et
  sous SPY en Sharpe ;
- donc le probleme restant n'est plus "pas les bonnes EMA" seulement. Il manque
  au moins un de ces elements : combinaison de plusieurs blocs EMA, contrainte
  secteur/asset, sizing dynamique selon qualite validation, ou objectif
  d'entrainement plus proche du portefeuille.

Objectif Optuna recommande pour cette famille :

```text
maximize:
  mean(validation_top20_excess, validation_top30_excess)
  - 0.5 * monthly_volatility(validation_top20_returns)
  - 0.5 * max_drawdown_penalty(validation_top20_returns)
```

Variantes a tester :

- objectif rendement : moyenne du futur rendement relatif du top K validation,
  avec `K in {10, 20, 30, 50}` ;
- objectif precision : part des actions du top K validation avec
  `future_excess_return > 0`, mais seulement comme objectif secondaire ;
- objectif risque : penaliser les folds ou le top K validation a un drawdown ou
  une volatilite trop forte ;
- ne pas optimiser RMSE/R2 comme objectif principal : ces metriques restent dans
  le rapport, mais elles ne mesurent pas la qualite du haut de portefeuille ;
- warm starts : conserver les meilleurs parametres par regime/objectif dans un
  JSON, puis les enqueuer avant les essais TPE/Bayesien.

Prochaine experience logique :

1. Ajouter un diagnostic `raw EMA exact rank` vs `boosting EMA exact prediction`
   pour voir si le booster degrade ou ameliore le classement brut de l'EMA.
2. Tester `validation_exact`, c'est-a-dire choisir chaque fold parmi les 43 EMA
   exactes Legacy observees sur validation passee, au lieu du seul couple
   dominant Legacy du mois.
3. Appliquer le risk overlay `dynamic_validation_quality_cash` au score
   `EMA exacte + residu`, car l'ancre exacte a des regimes de validation
   fortement negatifs, notamment 2019 et 2024.
...
- pas de choix automatique d'EMA, ni par validation, ni par dominance mensuelle
- modele base : boosting mlcraft regression sur cette EMA fixe et ses
  derives cross-sectionnels
- modele residuel : XGBoost avec `base_margin` egal a la prediction du modele
  EMA primaire. La raison technique est que le wrapper XGBoost actuel de
  `mlcraft` n'expose pas `base_margin`; le modele base reste mlcraft et le
  second etage utilise l'API XGBoost directe pour obtenir un vrai init score.
...
| Legacy `Combined_Equal` | 851.1% | 22.2% | 0.85 | -23.6% | 6.9% | 60.0% |
| SPY | 332.2% | 13.9% | 0.79 | -23.9% | 4.4% | 69.6% |
| EMA fixe + init-score top 20 | 312.1% | 13.4% | 0.41 | -38.3% | 8.1% | 58.5% |
| EMA fixe + init-score top 10 | 271.3% | 12.4% | 0.29 | -52.5% | 10.2% | 54.8% |
| EMA fixe + init-score top 30 | 244.3% | 11.6% | 0.36 | -42.0% | 7.8% | 60.0% |
| EMA fixe seule top 50 | 119.7% | 7.2% | 0.26 | -38.7% | 5.8% | 58.5% |
| EMA fixe seule top 20 | 81.7% | 5.5% | 0.16 | -36.6% | 6.3% | 58.5% |

Metriques prediction fixes sur test :

| modele | RMSE | MAE | R2 | Pearson | Spearman |
|---|---:|---:|---:|---:|---:|
| EMA fixe seule | 0.0778 | 0.0556 | -0.0068 | 0.0081 | 0.0241 |
| EMA fixe + init-score | 0.0778 | 0.0556 | -0.0067 | 0.0089 | 0.0226 |

Metriques top K fixes sur test :

| modele | top 10 excess | top 10 hit-rate | top 20 excess | top 20 hit-rate | top 50 excess | top 50 hit-rate |
|---|---:|---:|---:|---:|---:|---:|
| EMA fixe seule | -0.80% | 44.7% | -0.58% | 45.1% | -0.46% | 46.2% |
| EMA fixe + init-score | 0.15% | 47.6% | 0.09% | 47.6% | -0.27% | 46.6% |

Lecture :

- l'init-score fait exactement ce qui etait attendu techniquement : il corrige
  une ancre EMA tres faible et rend le top10/top20 positif en moyenne ;
- mais le couple fixe `34-7` seul est beaucoup moins bon que l'ancre Legacy
  dominante exacte du run `131004` ;
- le run fixe est tres regime-dependent : la validation top20 est negative sur
  de larges zones comme 2019 et 2024, puis redevient forte en 2021 et fin 2025 ;
- donc si Legacy impose un couple, il faut probablement aussi imposer une regle
  d'exposition/cash fondee sur la qualite validation recente de ce couple.
...
- principe : a chaque mois, prendre le couple EMA `n_short/n_long` du modele
  Legacy atomique dominant de ce mois, construire une colonne dynamique
  `legacy_exact_primary_mtr`, entrainer le modele base sur cette EMA primaire,
  puis entrainer le second etage avec `base_margin` egal a la prediction du
  modele EMA primaire.
- ce run est celui qui correspond au plan utilisateur : repeter chaque mois
  l'EMA que Legacy aurait prise, sans laisser le modele choisir l'EMA.

Backtest mensuel init-score :

| modele | rendement total | CAGR | Sharpe | max DD | vol mensuelle | mois positifs |
|---|---:|---:|---:|---:|---:|---:|
| Legacy `Combined_Frequency` | 1118.1% | 24.9% | 0.93 | -23.5% | 7.1% | 63.0% |
| Legacy `Combined_Equal` | 851.1% | 22.2% | 0.85 | -23.6% | 6.9% | 60.0% |
| EMA mensuelle + init-score top 10 | 475.6% | 16.8% | 0.40 | -51.3% | 10.8% | 55.6% |
| EMA mensuelle + init-score top 20 | 433.3% | 16.0% | 0.48 | -35.4% | 8.4% | 60.0% |
| SPY | 332.2% | 13.9% | 0.79 | -23.9% | 4.4% | 69.6% |
| EMA mensuelle + init-score top 30 | 326.2% | 13.8% | 0.43 | -39.4% | 7.9% | 58.5% |
| EMA mensuelle seule top 50 | 180.8% | 9.6% | 0.35 | -41.6% | 6.3% | 63.7% |
| EMA mensuelle seule top 20 | 149.8% | 8.5% | 0.29 | -36.4% | 6.5% | 60.0% |

Metriques prediction mensuelles init-score :

| modele | RMSE | MAE | R2 | Pearson | Spearman |
|---|---:|---:|---:|---:|---:|
| EMA mensuelle seule | 0.0778 | 0.0556 | -0.0066 | 0.0090 | 0.0243 |
| EMA mensuelle + init-score | 0.0778 | 0.0555 | -0.0065 | 0.0109 | 0.0241 |

Metriques top K mensuelles init-score :

| modele | top 10 excess | top 10 hit-rate | top 20 excess | top 20 hit-rate | top 50 excess | top 50 hit-rate |
|---|---:|---:|---:|---:|---:|---:|
| EMA mensuelle seule | -0.42% | 47.4% | -0.33% | 48.0% | -0.26% | 48.1% |
| EMA mensuelle + init-score | 0.51% | 50.5% | 0.29% | 49.7% | -0.04% | 48.3% |

Lecture :

- oui, ce run prend bien a chaque mois l'EMA qu'aurait prise Legacy, dans le
  sens du couple atomique Legacy dominant du mois ;
- le nom de colonne `legacy_exact_primary_mtr` reste identique, mais sa valeur
  est un switch mensuel vers le couple EMA exact du mois ;
- l'init-score n'apporte presque pas de difference numerique par rapport au run
  `131004`, mais il rend le protocole techniquement conforme : le second etage
  part bien de la prediction EMA ;
