...
Both consume the same retained data bytes and pass all seven required input
hashes, the same ten-ticker `historical_ticker_exclusions_v1` registry, and the
same `monthly_price_eligibility_v1` thresholds. The eligible replay is
`outputs/common_portfolio_replays/legacy_boosting_shared_eligibility_final_20260812`,
180 holding months from 2011-08 through 2026-07.

The current public report is generated by
`scripts/experiments/render_latest_common_dashboard.py` into
`outputs/research_dashboard/legacy_ema_alpha_shared_eligibility_final_20260812` and
...
historical evidence, not a promotion decision. New builders and renderers fail
closed when same-data, same-quarantine, or same-price-policy proof is absent.
...
price gate. The common replay is
`outputs/common_portfolio_replays/legacy_boosting_shared_eligibility_final_20260812`.

The comparable holding window is August 2011 through July 2026. CAGR is
24.2818% Top 10, 23.6497% Top 5, 17.0257% Legacy, and 14.3779% SPY total
return. See `docs/common_portfolio_backtest_engine.md` for the full risk table
and `docs/legacy_boosting_methodology.md` for pseudocode and ownership.

The final dashboard is
`outputs/research_dashboard/legacy_ema_alpha_shared_eligibility_final_20260812/html/alpharank_research_center.html`.
Start-year CAGR and annual returns are in
`outputs/research_dashboard/legacy_boosting_shared_eligibility_final_20260812/performance_by_year/performance_since_each_year.html`.
