...
- État : les 41 actions initiales sont traitées (3 validées, 38 implémentées), `RUN-006`/`RUN-007` sont validées provisoirement et `RUN-008`/`RUN-009` ont qualifié puis enregistré les 7 événements sélectionnés ; leur application au replay reste à faire, ainsi que la résolution ou l'approbation explicite des 656 cibles d'apprentissage provisoires avant promotion finale
...
| `RUN-009` | P0 | Construire un registre JSON unique, versionné et validé pour les événements `RUN-008`, avec un objet par événement et plusieurs preuves possibles par objet. Dépend de `RUN-008`. | `test_terminal_event_registry_is_complete_and_fail_closed` : schéma, identifiants, dates, montants, ratios, sources, SHA-256 et règle FRC sont validés ; toute ambiguïté bloque le chargement. | Validé | `RUN-009 (ce commit)` | Aucun avant intégration ; le registre projette 6 contreparties vers le contrat partagé et 1 blocage pré-exécution, sans modifier le snapshot ni les rendements par sa seule présence |
