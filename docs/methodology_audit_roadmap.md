...
- État : les 41 actions initiales sont traitées (3 validées, 38 implémentées) ; `RUN-010` à `RUN-012` ont scellé les prix successeurs, refusé FRC avant classement et résolu les 7 positions sélectionnées. Le replay commun `common_v2_terminal_resolved2` est comparable avec zéro position provisoire et `RUN-005` est scellé ; le seul verrou méthodologique restant est la résolution ou l'approbation explicite des 656 cibles d'apprentissage terminales avant promotion
...
| Exécution causale v2 | 12 | 12 | 0 | 0 | 3 | 9 | 75,0 % |
| **Total** | **53** | **24** | **22** | **7** | **41** | **12** | **22,6 %** |
