...
- État : les 41 actions initiales sont traitées (3 validées, 38 implémentées) ; `RUN-010` à `RUN-012` ont scellé les prix successeurs, refusé FRC avant classement et résolu les 7 positions sélectionnées. Le replay commun `common_v2_terminal_resolved2` est comparable avec zéro position provisoire, mais reste non promouvable tant que les 656 cibles d'apprentissage terminales ne sont pas résolues ou approuvées au niveau méthodologique et que `RUN-005` n'est pas relancé
...
| Exécution causale v2 | 12 | 12 | 0 | 0 | 4 | 8 | 66,7 % |
| **Total** | **53** | **24** | **22** | **7** | **42** | **11** | **20,8 %** |
...
| `v2-causal-terminal-resolved` | Comparable — non promouvable | Legacy + Boosting corrigés, positions sélectionnées résolues | `v2-causal-2975e65156b0-20260818T085346Z` | Blocage pré-exécution `9708152` ; contreparties `925f192` | `outputs/methodology_v2/run_004/common_v2_terminal_resolved2/manifest.json` ; holdings `56ed131b…05424` | Août 2011 à juillet 2026, 178 mois communs | `comparison_eligible=true`, zéro position provisoire et rapprochement `0` ; `promotion_eligible=false` jusqu'au traitement ou à l'approbation méthodologique des 656 cibles d'apprentissage et au nouveau `RUN-005` |
