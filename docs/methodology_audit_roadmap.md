...
- État : les 41 actions initiales sont traitées (3 validées, 38 implémentées) et les 13 actions d'exécution causale ont toutes une implémentation ou une preuve conservée. `RUN-010` à `RUN-012` ont scellé les prix successeurs, refusé FRC avant classement et résolu les 7 positions sélectionnées ; `RUN-013` formalise l'arbitrage humain sur les 656 cibles d'apprentissage terminales. Le replay `common_v2_approved_censoring1` est comparable et promouvable, sans position ni cible mature provisoire ; `RUN-005` est scellé et l'écart économique avec `v1` est entièrement expliqué. Les artefacts diagnostiques `RUN-006`/`RUN-007` restent conservés, volontairement provisoires, comme historique de la décision. Vingt actions de go-live, structuration des données et publication sont désormais suivies sans supprimer l'historique. `LIVE-006` archive chaque observation Yahoo sous forme de changements immuables sans réécrire les lignes identiques et fournit le registre immuable EODHD. `LIVE-007` construit `STG/DEF`, applique la reprise sourcée de la même clé et interdit tout report entre dates ; un essai hors ligne sur 571 900 lignes a reconstruit 260 absences sans aucun écart de valeur. `LIVE-008` a catalogué les 49 fichiers EODHD locaux sans les réécrire, matérialisé les couches `STG/DEF/MART`, prouvé l'identité des neuf fichiers modèle et basculé atomiquement le pointeur vers le `MART` pour la composition validée `2a01288b…9be9`. `LIVE-009` branche cette reprise de clés identiques dans le chemin de production tout en exigeant au moins une observation nouvelle pour chaque titre actif. `LIVE-010` gèle le préfixe déjà validé d'un titre lorsque son prétendu téléchargement complet reste incomplet, conserve le téléchargement fautif en RAW et n'ajoute que les nouvelles dates. `LIVE-011` distingue un vrai mouvement de marché extrême d'une rupture de prix au moyen d'un registre borné par titre, date, cours, rendement, sources et hash. `LIVE-012` transmet cette résolution auditée au constructeur du package prix final, sans faire passer les lignes reprises pour un nouveau téléchargement. `LIVE-013` empêche une sortie d'indice déjà sourcée de vieillir artificiellement la fraîcheur des titres encore négociables, tout en conservant le titre terminal et sa dernière cotation dans l'historique. `LIVE-014` exige désormais une justification durable avant toute migration de comptes historiques. `LIVE-015` rend canoniques les chemins de données lorsqu'un run part d'un worktree propre. `LIVE-016` isole la revue du premier package SEC-only construit par le code causal effectivement commité. `LIVE-017` sépare le panier immédiatement négociable de la comparaison économique, qui ne peut compter que des mois entièrement terminés. `LIVE-018` interdit de sélectionner pour un nouveau mois un titre dont la fin de cotation est déjà connue, sans toucher aux positions achetées avant l'événement. Le go-live mensuel AlphaRank attend encore les replays et contrôles finaux sous `LIVE-003` ; IBKR est explicitement hors gate AlphaRank.
...
| `LIVE-003` | P0 | Exécuter l'ingestion complète, construire les packages prix gardé et SEC-only, composer le snapshot, lancer Legacy depuis un worktree propre puis valider le replay strict avant le rééquilibrage. | Manifests de fraîcheur et de lineage verts, zéro clé prix retirée, scope `full_ingestion`, cutoff de mois complet, replay strict et comparaison avec le dernier portefeuille publié. | Bloqué — chemin runtime du worktree | — | Run `20260820_001432` : données et revue historique passées, puis rollback transactionnel à l'écriture du manifeste parce que le chemin logique du worktree et le chemin réel du store ont été comparés sans normalisation. Tous les téléchargements et rapports restent conservés ; l'actif `20260811_001503` est inchangé. |
...
| `LIVE-011` | P0 | Autoriser un mouvement journalier supérieur au seuil automatique seulement s'il correspond exactement à un registre revu et versionné : titre, date, fourchettes du cours précédent, du nouveau cours et du rendement, heure de connaissance, motif, sources et SHA-256. | Run réel `20260819_215259` : MRNA passe de `62,959999` à `174,380005`, soit `+176,9695 %`, avec 185 077 360 titres ; annonce Phase 3 INTerpath-001 et reporting de marché concordants. Toute valeur hors des bornes `62,95–62,97`, `174,37–174,39` et `176,9–177,1 %` reste bloquante. 30 tests ciblés puis suite production de 222 tests verts ; validation finale attendue sous `LIVE-003`. | Implémenté | `95e7df0acc880c6b284f34b81b34ab9a88bdb331` | Le vrai rendement MRNA du 19 août est conservé ; aucun assouplissement générique du seuil de 40 % et aucun effet sur les dates ou titres non inscrits. |
| `LIVE-012` | P0 | Transmettre au constructeur du package prix canonique l'identifiant du run d'ingestion qui a résolu le millésime Yahoo. Les lignes historiques reprises gardent leur ancien identifiant ; chaque titre actif doit néanmoins avoir au moins une ligne du run courant. Dépend de `LIVE-009` et `LIVE-010`. | Le constructeur accepte un millésime contenant une ligne antérieure strictement auditée et une ligne courante, transmet le même identifiant au compositeur et au garde final, refuse un fichier sans observation du run annoncé et ne prétend pas qu'un historique repris provient intégralement du nouveau millésime. 13 tests ciblés verts ; validation réelle attendue sous `LIVE-003`. | Implémenté | `b5effc76752b6b4cadd11091ad24881488f453b0`, `92150b9fe6db23d9517bb0ee6860870dcabed244` | Aucun prix ne change par ce raccord. Il évite que le package final refuse à tort les préfixes AVB/EQR déjà validés, sans rebaptiser ces anciennes lignes en données nouvelles. |
| `LIVE-013` | P0 | Calculer la fraîcheur des titres encore négociables sans laisser un membre du dernier snapshot mensuel, mais déjà couvert par une sortie d'indice sourcée et effective, imposer sa dernière cotation au reste de l'univers. Conserver séparément le nombre et la liste de ces exclusions terminales. | Run `20260819_220746` : les 503 membres d'août comprennent EA, sortie effective le 5 août et dernière cotation conservée le 10 ; les 502 titres encore négociables atteignent le 18 août au minimum. Test synthétique fail-closed sans registre puis vert avec EA ; 31 tests fraîcheur/ingestion/prix verts. Validation réelle attendue sous `LIVE-003`. | Implémenté | `599aff0fa450b18b7b3bb4755b0e983649c31fcf` | Aucun prix EA n'est supprimé ni prolongé artificiellement. Sa dernière cotation et sa sortie restent visibles, tandis que la fraîcheur commune des 502 titres actifs passe du 10 au 18 août. |
| `LIVE-014` | P0 | Rendre toute migration de fondamentaux historiques explicitement revue : le drapeau d'exception seul ne suffit pas, une justification non vide doit être conservée dans le rapport et le manifeste. Utiliser ce mécanisme une seule fois pour aligner le store mixte encore daté du 11 août sur la migration SEC déjà revue le 16 août, sans supprimer l'ancien package. | Tests : refus avant publication si l'exception n'a pas de note ; note reproduite dans le garde et le contrat ; run réel avec rapport exhaustif et ancien snapshot conservé ; les runs ordinaires suivants gardent l'exception désactivée. | Implémenté | `e85ddc671c41e8a563c9a652f8d5039e18fca73f` | Aucun tant que la revue réelle n'est pas exécutée. Le run `20260819_231418` montre le décalage : le contrôle compare encore au package mixte `20260811_001503`, antérieur à la migration de versions de filings déjà scellée dans le package SEC-only du 16 août. |
| `LIVE-015` | P0 | Normaliser tous les chemins du store de données vers leur cible réelle lorsqu'une ingestion est lancée depuis un worktree propre dont `data/` est un lien. Les caches, audits, registres RAW, manifests et snapshots doivent partager la même racine canonique, tout en gardant le code et les configurations dans le worktree scellé. | Test avec `data/` symbolique : racines officielle, audit, référence, warehouse et dernier snapshot toutes résolues vers le store réel ; run réel écrit son manifeste sans `ValueError`, puis le validateur confirme les mêmes hashes et le rollback reste intact sur erreur. | Implémenté | `f1e6a434debaef919bc10c700272acfce998b07b` | Aucun effet économique. Le run `20260820_001432` a prouvé que tous les contrôles de données étaient passés avant l'échec de sérialisation du chemin d'audit ; la transaction a restauré les hashes actifs du 11 août. |
...
| 21 | `LIVE-011` | `fix(LIVE-011): admit reviewed MRNA market move` |
| 22 | `LIVE-012` | `fix(LIVE-012): propagate audited price resolution` |
| 23 | `LIVE-013` | `fix(LIVE-013): exclude sourced terminal members from freshness` |
| 24 | `LIVE-014` | `fix(LIVE-014): require documented fundamental revision approval` |
| 25 | `LIVE-015` | `fix(LIVE-015): canonicalize clean-worktree data paths` |
...
| 2026-08-19 | `LIVE-011` | Codex | Gate G8 | À faire | Implémenté | `95e7df0acc880c6b284f34b81b34ab9a88bdb331` | Run rejeté `20260819_215259` : garde de révision historique vert, puis MRNA `62,959999 -> 174,380005`, volume `185 077 360` ; registre `reviewed_extreme_price_moves_v1` hashé et contrôle réel du run vert ; suite production 222 tests verte | La hausse de marché liée aux résultats Phase 3 est acceptée dans des bornes étroites. Le garde continue de refuser tout autre saut extrême, une autre date ou une valeur MRNA différente. |
| 2026-08-20 | `LIVE-012` | Codex | Gate G8 | À faire | Implémenté | `b5effc76752b6b4cadd11091ad24881488f453b0` | 13 tests ciblés verts ; le run annoncé doit être présent dans le millésime et son identifiant est transmis aux deux contrôles du package canonique | Les reprises AVB/EQR restent attribuées à leur run d'origine ; elles sont acceptées uniquement parce que chaque titre possède aussi une observation du run courant. |
| 2026-08-20 | `LIVE-012` | Codex | Gate G8 | Implémenté | Implémenté | `92150b9fe6db23d9517bb0ee6860870dcabed244` | Le manifeste distingue désormais un historique actif entièrement neuf d'une résolution auditée qui conserve des lignes anciennes ; 13 tests ciblés toujours verts | Le contrôle reste vert, mais le manifeste ne peut plus affirmer à tort que toutes les lignes actives appartiennent à un seul millésime. |
| 2026-08-20 | `LIVE-013` | Codex | Gate G8 | À faire | Implémenté | `599aff0fa450b18b7b3bb4755b0e983649c31fcf` | Run rejeté `20260819_220746` conservé : réseau complet, 503 membres, EA seul minimum au 10 août ; reconstruction avec exclusion terminale sourcée : 502 titres négociables, zéro manquant, minimum au 18 août ; 31 tests ciblés verts | Le garde reste strict pour tout titre non terminal. EA demeure dans les prix et le manifeste, avec une exclusion de fraîcheur explicite au lieu d'un cours inventé après sa sortie. |
| 2026-08-20 | `LIVE-003` | Codex | Gate G8 | Bloqué — fournisseur prix | Bloqué — migration fondamentale à revoir | — | Run `20260819_231418` : 732 titres téléchargés, prix et SEC complets, prix gardés sans suppression ; arrêt avant publication sur `income_statement`, `balance_sheet`, `cash_flow`, `shares` et `earnings` historiques | Le garde fonctionne. La baseline mixte active reste `20260811_001503`, donc antérieure à la migration SEC versionnée et déjà approuvée du 16 août ; aucun override silencieux n'est appliqué. |
| 2026-08-20 | `LIVE-014` | Codex | Gate G8 | — | À faire | — | Nouveau contrôle demandé par le run `20260819_231418` : associer toute exception historique à une justification persistée, puis conserver le rapport de migration et l'ancien package | L'exception doit rester désactivée après la migration unique ; une simple variable booléenne ne constitue pas une revue auditable. |
| 2026-08-20 | `LIVE-014` | Codex | Gate G8 | À faire | Implémenté | `e85ddc671c41e8a563c9a652f8d5039e18fca73f` | 21 tests ciblés verts ; le garde conserve `revision_review_note` et `approval_recorded`, le CLI et le run nocturne refusent l'exception avant tout réseau si la justification manque | La validation attend le run de migration réel et la preuve que l'ancien package du 11 août reste intact dans l'historique. |
| 2026-08-20 | `LIVE-003` | Codex | Gate G8 | Bloqué — migration fondamentale à revoir | Bloqué — chemin runtime du worktree | — | Run `20260820_001432` : revue `LIVE-014` présente et cinq familles historiques autorisées, puis erreur de chemin audit lors de la sérialisation finale ; rollback vérifié par les hashes prix `b4f81b5d…30ac8` et résultat `540f307a…385e` | Le défaut vient du mélange entre `/private/tmp/.../data` et `/Users/.../alpharank/data`; aucune donnée active ni aucun snapshot n'a été remplacé. |
| 2026-08-20 | `LIVE-015` | Codex | Gate G8 | — | À faire | — | Normaliser la racine réelle du store avant de construire les chemins audit, cache, warehouse et manifeste ; couvrir le cas symlink par test | Le code reste celui du worktree propre, mais toutes les écritures de données doivent viser une seule racine physique. |
| 2026-08-20 | `LIVE-015` | Codex | Gate G8 | À faire | Implémenté | `f1e6a434debaef919bc10c700272acfce998b07b` | 22 tests ciblés verts ; le cas `data/` symbolique vérifie la même racine physique pour official, audit, référence, warehouse et manifest, et reproduit le `relative_to` final sans erreur | Les configurations restent lues depuis le commit propre ; seuls les chemins de données sont normalisés vers le store persistant. La validation réelle reste sous `LIVE-003`. |
