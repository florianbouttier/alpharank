# Archive des instructions agents avant consolidation — 20 août 2026

> Contenu conservé intégralement pour audit. La source normative courante est
> `AGENTS.md` à la racine ; ce fichier archivé ne doit plus être modifié.
...
- Before committing or pushing a production-data, Legacy, or Boosting change,
  run the verification suite documented in `docs/monthly_portfolio_runbook.md`
  and the relevant data/methodology contracts, plus the strict Legacy replay
  validator when a new monthly run is part of the change. Never describe a change as
...
- Before adding or substantially modifying Python, data pipelines, schemas, or
  repository structure, read and follow the relevant file under
  `docs/standards/`. These standards apply immediately to new work; migrate
  historical violations only through isolated roadmap tasks and never through
  an unrelated bulk reformat.
- Keep documentation centralized. Do not create scattered one-off notes when a canonical note exists.
- `README.md` is the five-minute onboarding entrypoint, `ROADMAP.md` tracks the
  repository cleanup, `CONTRIBUTING.md` defines the active development
  standards, and `docs/README.md` is the canonical documentation index.
  `docs/archive/CODEX_HANDOFF.md` is historical evidence, never an active
  source of truth.
- Every maintained source, configuration, test, or application directory must
  contain a short `README.md` explaining its responsibility, immediate child
  directories, ownership boundaries, and canonical docs. Generated snapshots,
  caches, environments, logs, and output run folders are exempt and must be
  explained by their nearest maintained parent README instead.
- Run `scripts/validate_documentation.py` whenever directories or Markdown
  links change. Do not publish the site if that validation fails.
- Current cross-track state belongs in the relevant canonical contract and
  roadmap, both linked from `docs/README.md`; do not append new current-state
  sections to the archived Codex handoff.
- For boosting/Legacy-copy R&D, update
  `docs/research/boosting_signal_copy_model_catalog.md`; do not create parallel
  experiment notes unless they are linked from the research index and
...
- Every public cross-method performance report must show, for each strategy and
  SPY: CAGR from January 1 of every requested start year from 2010 through the
  current year, calendar-year returns, maximum drawdown, Legacy-convention
  Sharpe, worst complete calendar year, and worst realized month. A requested
  start before the first common out-of-sample month must be labelled with its
  effective start and must never be backfilled.
...
- Prefer project-visible memory in `AGENTS.md`, the root roadmaps, and relevant
  canonical docs over relying on chat history. `AGENT.md` is only a
  compatibility pointer and must not duplicate rules.
...
`AGENT.md` is a compatibility pointer to this file. The former long guide is
preserved under `docs/archive/agent_guides/`; do not recreate a second active
instruction set.
