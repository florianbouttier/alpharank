# Archive du guide agent AlphaRank au 20 août 2026

> Ce document est conservé pour ne perdre aucune consigne historique. Pour une
> nouvelle session, la seule source active est [`../../../AGENTS.md`](../../../AGENTS.md).
...
Onboarding rule: `README.md` is the five-minute entrypoint, `docs/README.md` is
the documentation map, and local folder READMEs explain ownership without
duplicating methodology. Generated snapshots, outputs, logs and caches are
documented by their maintained parent. Validate this structure with
...
- Public cross-method reports must show CAGR from January 1 of every requested start year from 2010 through the current year, calendar-year returns, maximum drawdown, Legacy-convention Sharpe, worst complete calendar year, and worst realized month for every strategy and SPY. Label the effective common start when out-of-sample history begins later; never backfill it.
...
- If a component is deprecated, preserve its history in Git and document its
  replacement; do not invent a new archive directory without a clear owner.
