# Archive du README AlphaRank au 20 août 2026

> Document conservé avant la réduction de l'onboarding racine. Les commandes et
> contrats courants sont à vérifier dans [`../../README.md`](../../README.md) et
> [`../README.md`](../README.md).
...
- **Legacy**: baseline de production mensuelle.
- **Boosting**: challenger XGBoost évalué en walk-forward.

Les signaux restent séparés, mais les deux méthodes utilisent le même moteur de
portefeuille, le même snapshot et le même benchmark pour toute comparaison
publiée.

## Start Here

Pour comprendre le dépôt sans lire son historique complet :

1. ce README donne l'orientation et les commandes ;
2. [`docs/README.md`](../README.md) est le sommaire documentaire canonique ;
3. [`docs/monthly_portfolio_runbook.md`](../monthly_portfolio_runbook.md)
   décrit le run mensuel ;
4. [`docs/legacy_boosting_methodology.md`](../legacy_boosting_methodology.md)
   explique les deux algorithmes ;
5. chaque dossier actif possède un `README.md` local qui décrit sa
   responsabilité et ses enfants.

`docs/CODEX_HANDOFF.md` conserve le journal détaillé des décisions et incidents.
Il sert au diagnostic, pas à l'onboarding initial.
...
data/                     # sources et snapshots locaux, majoritairement ignorés
docs/                     # contrats canoniques, état courant et archives datées
scripts/                  # entrées CLI et orchestration
src/alpharank/            # code Python réutilisable
tests/                    # tests unitaires, intégration et replay
outputs/                  # artefacts de runs, ignorés par Git
logs/                     # journaux, ignorés par Git
```

Voir [`src/alpharank/README.md`](../../src/alpharank/README.md) pour le détail des
packages et [`scripts/README.md`](../../scripts/README.md) pour les points d'entrée.
...
Ne pas recopier un compteur de tests dans une conclusion : exécuter la suite
ciblée ou complète et reporter le résultat du run courant. Le package Legacy de
référence passe également la validation stricte de replay.
...
- Keep each active source/configuration folder's local `README.md` current.
- Run `./.venv/bin/python scripts/validate_documentation.py` after changing the
  repository structure or documentation links.
